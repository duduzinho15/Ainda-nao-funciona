...
def get_ofertas_hoje() -> int:
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM ofertas WHERE DATE(data_postagem) = ?", (hoje,)
        )
        total = cursor.fetchone()[0]
...
def get_lojas_ativas() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for row in cursor.fetchall():
            lojas.append(
                {"nome": row[0] or "Loja não identificada", "contagem": row[1]}
            )
...
def lojas():
    ...
    return render_template("lojas.html", lojas=lojas_ativas, total_lojas=total_lojas)
...
def ofertas_hoje():
    ...
    return render_template(
        "ofertas_hoje.html", ofertas=ofertas_hoje, total_ofertas=len(ofertas_hoje)
    )
