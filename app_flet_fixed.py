...
class GeekAlertFletApp:
    ...
    def show_config(self, e):
        ...
        try:
            # Seleciona a aba de configurações (índice 1)
            if hasattr(self.page, 'views') and len(self.page.views) > 0:
                main_view = self.page.views[0]
                if hasattr(main_view, 'controls') and main_view.controls and len(main_view.controls) > 1:
                    main_panel = main_view.controls[1]
                    if hasattr(main_panel, 'controls') and main_panel.controls and len(main_panel.controls) > 1:
                        tabs = main_panel.controls[1]
                        if hasattr(tabs, 'selected_index'):
                            tabs.selected_index = 1
                            self.page.update()
                            logger.info("✅ Aba de configurações selecionada")
                        else:
                            logger.warning("⚠️ Tabs não têm selected_index")
                    else:
                        logger.warning("⚠️ Main panel não tem controles suficientes")
                else:
                    logger.warning("⚠️ Main view não tem controles suficientes")
            else:
                logger.warning("⚠️ Página não tem views")
        except Exception as e:
    ...
    def show_stats(self, e):
        ...
        try:
            ...
            # Usa método alternativo para diálogos
            if hasattr(self.page, 'dialog'):
                self.page.dialog = stats_dialog
                self.page.dialog.open = True
            else:
                # Fallback: adiciona diretamente à página
                self.page.add(stats_dialog)
            
            self.page.update()
            logger.info("✅ Diálogo de estatísticas aberto")
    ...
    def close_dialog(self):
        ...
        try:
            if hasattr(self.page, 'dialog') and self.page.dialog:
                ...
                # Remove o último controle adicionado (diálogo)
    ...
    def clear_logs_display(self, e=None):
        ...
        try:
            ...
            logger.error(f"❌ Erro ao limpar display de logs: {e}")
    ...
    def show_snackbar(self, message: str, color: str):
        ...
        try:
            # Verifica se o método existe na versão atual do Flet
            if hasattr(self.page, 'show_snack_bar'):
                ...
                # Fallback: mostra no log se snackbar não estiver disponível
    ...
    def update_logs(self):
        ...
        try:
            ...
            # Agenda próxima atualização usando threading
            import threading
            import time
            ...
            def schedule_next_update():
                ...
                if hasattr(self, 'page') and self.page:
                    try:
                        # Tenta usar page.after se disponível
    ...
    def main(self, page: ft.Page):
        ...
        # Usa métodos alternativos para configuração de janela
        try:
