#!/usr/bin/env python3
"""
Scraper da Shopee usando Playwright - Versão 3.0 com Tratamento de Bloqueios
Lida com autenticação, captchas e bloqueios anti-bot
"""
...
class ShopeePlaywrightScraperV3(BasePlaywrightScraper):
    ...
    async def check_page_status(self, page_content: str) -> str:
        """Verifica o status da página e identifica bloqueios"""
        try:
            ...
            if "captcha" in page_content.lower() or "verificação" in page_content.lower():
            ...
            # Verifica se é página de resultados
            ...
            logger.error(f"❌ Erro ao verificar status da página: {e}")
            return "ERROR"
    
    async def handle_login_page(self):
        """Tenta contornar a página de login"""
        try:
            logger.info("🔐 Detectada página de login, tentando contornar...")
            
            # Tenta acessar diretamente a página inicial
            ...
            if "Entre" in page_content:
                logger.warning("⚠️ Ainda na página de login, tentando outras estratégias...")
                
                # Tenta acessar uma categoria específica
                await self.page.goto(f"{self.base_url}/category/11013230", wait_until="networkidle")
                await asyncio.sleep(3)
                
                # Verifica novamente
                page_content = await self.page.content()
                if "Entre" in page_content:
                    logger.error("❌ Não foi possível contornar a página de login")
                    return False
                else:
                    logger.info("✅ Conseguiu contornar a página de login")
                    return True
            else:
                logger.info("✅ Conseguiu contornar a página de login")
                return True
            ...
            logger.error(f"❌ Erro ao tentar contornar login: {e}")
            return False
    
    async def handle_captcha(self):
        """Tenta resolver captcha ou aguarda resolução manual"""
        try:
            logger.info("🤖 Detectado captcha, aguardando resolução manual...")
            
            # Aguarda até 60 segundos para resolução manual
            for i in range(60):
                ...
                if "captcha" not in page_content.lower():
                    logger.info("✅ Captcha parece ter sido resolvido")
                    return True
                
                if i % 10 == 0:  # Log a cada 10 segundos
                    logger.info(f"⏳ Aguardando resolução do captcha... ({i}s)")
            
            logger.warning("⚠️ Timeout aguardando captcha")
            return False
            
        except Exception as e:
            logger.error(f"❌ Erro ao lidar com captcha: {e}")
            return False
    
    async def search_products_alternative(self, categoria: str) -> list:
        """Método alternativo de busca que não depende de login"""
        try:
            logger.info(f"🔍 Tentando busca alternativa para: {categoria}")
            
            # Tenta acessar categoria direta
            category_url = f"{self.base_url}/category/11013230"  # Categoria de smartphones
            ...
            if status == "LOGIN_REQUIRED":
                logger.warning("⚠️ Categoria também requer login")
                return []
            ...
            # Procura por produtos usando seletores genéricos
            products = []
    ...
    async def debug_page_structure(self, categoria: str):
        ...
        try:
            ...
            page_content = await self.page.content()
            status = await self.check_page_status(page_content)
            logger.info(f"📊 Status da página: {status}")
            
            if status == "LOGIN_REQUIRED":
                logger.warning("⚠️ Página requer login, tentando contornar...")
                if not await self.handle_login_page():
                    logger.error("❌ Não foi possível contornar o login")
                    return
                ...
                page_content = await self.page.content()
                status = await self.check_page_status(page_content)
                logger.info(f"📊 Novo status da página: {status}")
            
            if status == "CAPTCHA_REQUIRED":
                logger.warning("⚠️ Captcha detectado, tentando resolver...")
    ...
    async def buscar_ofertas_gerais(self):
        ...
        try:
            logger.info("🚀 INICIANDO BUSCA DE OFERTAS NA SHOPEE COM PLAYWRIGHT V3.0")
            logger.info("=" * 60)
            ...
            for categoria in self.categorias:
                try:
                    ...
                    # Se não encontrou produtos, tenta método alternativo
                    if not ofertas_categoria:
                        logger.info(f"🔄 Tentando método alternativo para {categoria}")
                        ofertas_categoria = await self.search_products_alternative(categoria)
...
async def main():
    ...
    print("🚀 TESTANDO SHOPEE SCRAPER COM PLAYWRIGHT V3.0 (TRATAMENTO DE BLOQUEIOS)")
    print("=" * 60)
    
    scraper = ShopeePlaywrightScraperV3(headless=False)  # Modo visível para debug
