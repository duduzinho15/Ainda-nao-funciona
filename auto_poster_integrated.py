#!/usr/bin/env python3
"""
Sistema Integrado de Postagem Automática
...
class AutoPosterIntegrated:
    """Sistema integrado de postagem automática de ofertas"""
    
    def __init__(self):
        self.orchestrator = IntelligentScraperOrchestrator()
        self.posted_products = set()  # Cache de produtos já postados
        self.collected_products = []  # Produtos coletados do último ciclo
        self.posting_interval = timedelta(minutes=30)  # Intervalo entre postagens
        self.max_posts_per_cycle = 5  # Máximo de posts por ciclo
        self.running = False
    ...
    async def start(self):
        """Inicia o sistema de postagem automática"""
        logger.info("🚀 Iniciando Sistema Integrado de Postagem Automática")
        self.running = True
    ...
    async def run_scraping_cycle(self):
        ...
        logger.info("🔄 Iniciando ciclo de scraping e postagem")
        ...
        try:
            ...
            logger.info("✅ Ciclo de scraping e postagem concluído")
            
        except Exception as e:
            logger.error(f"❌ Erro no ciclo de scraping e postagem: {e}")
        ...
        try:
            ...
            for scraper_id, scraper_config in self.orchestrator.scrapers.items():
                if scraper_config.enabled:
                    try:
                        logger.info(f"📦 Executando scraper: {scraper_config.name}")
                        
                        # Executa o scraper
                        products = await self.orchestrator.run_scraper_function(scraper_config.function_name)
                        
                        if products:
                            # Adiciona informações de fonte aos produtos
                            for product in products:
                                product['fonte'] = scraper_config.name
                                product['loja'] = product.get('loja', 'Loja não informada')
                            
                            # Filtra produtos do Buscapé (usado apenas para histórico de preços)
                            # Remove acentos para comparação
                            import unicodedata
                            scraper_name_normalized = ''.join(
                                c for c in unicodedata.normalize('NFD', scraper_config.name.lower())
                                if not unicodedata.combining(c)
                            )
                            
                            if 'buscape' not in scraper_name_normalized:
                                all_products.extend(products)
                                logger.info(f"✅ {scraper_config.name}: {len(products)} produtos coletados")
                            else:
                                logger.info(f"📊 {scraper_config.name}: {len(products)} produtos coletados (apenas para histórico de preços)")
                        else:
                            logger.warning(f"⚠️ {scraper_config.name}: Nenhum produto coletado")
                            
                    except Exception as e:
                        logger.error(f"❌ Erro ao executar {scraper_config.name}: {e}")
                        continue
                    
                    # Aguarda um pouco entre scrapers para não sobrecarregar
                    await asyncio.sleep(2)
            
            logger.info(f"📊 Total de produtos coletados para postagem: {len(all_products)}")
            return all_products
    ...
    def filter_products_for_posting(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Filtra produtos para postagem baseado em critérios"""
        filtered_products = []
        ...
        for product in products:
            ...
            # Verifica se é produto geek/tech (filtro básico)
            if not self.is_geek_product(product):
                continue
            
            # Para scrapers agregadores (Promobit, Pelando, MeuPC.net), 
            # filtra apenas produtos de lojas com afiliação
            source = product.get('fonte', '').lower()
            if source in ['promobit', 'pelando', 'meupc.net']:
                if not self.is_affiliated_store(product):
                    logger.debug(f"Produto de {source} ignorado - loja sem afiliação: {product.get('loja', 'N/A')}")
                    continue
            
            filtered_products.append(product)
        
        # Ordena por relevância (preço, avaliação, etc.)
        filtered_products.sort(key=lambda p: self.calculate_relevance_score(p), reverse=True)
        
        # Limita quantidade de produtos para postar
        filtered_products = filtered_products[:self.max_posts_per_cycle]
        
        logger.info(f"🎯 {len(filtered_products)} produtos filtrados para postagem")
        return filtered_products
    
    def is_valid_product(self, product: Dict[str, Any]) -> bool:
        """Verifica se um produto tem informações válidas para postagem"""
        ...
        # Verifica se o título tem tamanho mínimo
        if len(str(product.get('titulo', ''))) < 10:
            return False
        
        # Verifica se o preço é válido
        price = str(product.get('preco', ''))
        if not price or price == 'Preço não informado':
            return False
        
        return True
    
    def is_geek_product(self, product: Dict[str, Any]) -> bool:
        """Verifica se um produto é considerado geek/tech"""
        title = str(product.get('titulo', '')).lower()
        
        # Palavras-chave geek/tech
        geek_keywords = [
            'smartphone', 'celular', 'iphone', 'samsung', 'xiaomi',
            'notebook', 'laptop', 'pc', 'computador', 'gamer',
            'console', 'ps5', 'xbox', 'nintendo', 'switch',
            'tv', 'smart tv', '4k', 'oled', 'qled',
            'tablet', 'ipad', 'android',
            'headphone', 'fone', 'airpods', 'bluetooth',
            'smartwatch', 'relógio', 'fitness',
            'câmera', 'gopro', 'canon', 'nikon',
            'drone', 'quadricóptero',
            'gaming', 'mouse', 'teclado', 'headset',
            'monitor', 'led', 'curvo',
            'impressora', '3d', 'laser',
            'robo', 'automático', 'inteligente'
        ]
        
        # Verifica se contém palavras-chave geek
    ...
    async def post_offers(self, products: List[Dict[str, Any]]):
        ...
        if not products:
            logger.info("📭 Nenhum produto para postar")
            return
        
        logger.info(f"📤 Iniciando postagem de {len(products)} ofertas")
        ...
        for i, product in enumerate(products):
            try:
                ...
                if success:
                    ...
                    logger.info(f"✅ Oferta {i+1}/{len(products)} postada com sucesso: {product['titulo'][:50]}...")
                else:
                    self.stats["failed_posts"] += 1
                    logger.error(f"❌ Falha ao postar oferta {i+1}/{len(products)}")
                
                # Delay entre postagens
                await asyncio.sleep(10)
                
            except Exception as e:
                logger.error(f"❌ Erro ao postar oferta {i+1}/{len(products)}: {e}")
                self.stats["failed_posts"] += 1
                continue
        
        self.stats["total_posts"] += len(products)
        logger.info(f"📊 Postagem concluída: {len(products)} ofertas processadas")
    
    def format_offer_message(self, product: Dict[str, Any]) -> str:
        """Formata mensagem da oferta para o Telegram"""
        title = product.get('titulo', 'Produto sem título')
        price = product.get('preco', 'Preço não informado')
        original_price = product.get('preco_original')
        store = product.get('loja', 'Loja não informada')
        source = product.get('fonte', 'Fonte não informada')
    ...
    def generate_affiliate_url(self, product: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if not original_url:
                logger.warning(f"Produto sem URL: {product.get('titulo', 'Sem título')}")
                return None
            ...
            elif 'mercado livre' in source or 'mercado livre' in store or 'mercadolivre' in store:
                return self._generate_mercadolivre_affiliate_url(original_url)
            elif any(awin_store in store for awin_store in ['kabum', 'dell', 'lenovo', 'acer', 'asus', 'samsung']):
                return self._generate_awin_affiliate_url(original_url, store)
            else:
                # Para outras lojas, retorna URL original
                logger.info(f"Loja não suportada para afiliados: {store}")
                return original_url
    ...
    def _generate_amazon_affiliate_url(self, original_url: str) -> str:
        ...
        try:
            ...
            if not hasattr(config, 'AMAZON_ASSOCIATE_TAG') or not config.AMAZON_ASSOCIATE_TAG:
                logger.warning("AMAZON_ASSOCIATE_TAG não configurado")
                return original_url
            ...
            if '/dp/' in original_url:
                ...
                # Se não conseguir extrair ASIN, adiciona tag de afiliado à URL original
                separator = '&' if '?' in original_url else '?'
                affiliate_url = f"{original_url}{separator}tag={config.AMAZON_ASSOCIATE_TAG}"
    ...
    def _generate_aliexpress_affiliate_url(self, original_url: str) -> str:
        ...
        try:
            ...
            # Verifica se as credenciais do AliExpress estão configuradas
            if not hasattr(config, 'ALIEXPRESS_TRACKING_ID') or not config.ALIEXPRESS_TRACKING_ID:
                logger.warning("ALIEXPRESS_TRACKING_ID não configurado")
                return original_url
            
            # Adiciona parâmetros de afiliado à URL do AliExpress
            separator = '&' if '?' in original_url else '?'
    ...
    def _generate_shopee_affiliate_url(self, original_url: str) -> str:
        ...
        try:
            ...
            if not hasattr(config, 'SHOPEE_PARTNER_ID') or not config.SHOPEE_PARTNER_ID:
                logger.warning("SHOPEE_PARTNER_ID não configurado")
                return original_url
            
            # Adiciona parâmetros de afiliado à URL da Shopee
            separator = '&' if '?' in original_url else '?'
            affiliate_url = f"{original_url}{separator}affiliate_id={config.SHOPEE_PARTNER_ID}"
    ...
    def _generate_mercadolivre_affiliate_url(self, original_url: str) -> str:
        ...
        try:
            ...
            logger.warning("Sistema de afiliados Mercado Livre não disponível")
            return original_url
        except Exception as e:
    ...
    def _generate_awin_affiliate_url(self, original_url: str, store: str) -> str:
        ...
        try:
            ...
            # Verifica se as credenciais da Awin estão configuradas
            if not hasattr(config, 'AWIN_PUBLISHER_ID') or not config.AWIN_PUBLISHER_ID:
                logger.warning("AWIN_PUBLISHER_ID não configurado")
                return original_url
            
            # Mapeia lojas para IDs da Awin (será atualizado dinamicamente)
            store_mapping = {
                'kabum': 'kabum',
                'dell': 'dell',
                'lenovo': 'lenovo',
            ...
            if store_key:
                # Adiciona parâmetros de afiliado da Awin
                separator = '&' if '?' in original_url else '?'
                ...
                logger.warning(f"Loja {store} não mapeada para Awin")
                return original_url
    ...
    def is_affiliated_store(self, product: Dict[str, Any]) -> bool:
        """Verifica se o produto é de uma loja com afiliação"""
        store = product.get('loja', '').lower()
        
        # Lojas com afiliação confirmada
        affiliated_stores = [
            'amazon', 'aliexpress', 'shopee', 'mercado livre', 'mercadolivre',
            'magazine luiza', 'magalu', 'kabum', 'dell', 'lenovo', 'acer',
            'asus', 'samsung', 'trocafy', 'casa bahia'
        ]
        
        # Verifica se a loja está na lista de afiliados
    ...
    def print_status(self):
        ...
        print("\n" + "="*60)
        print("🤖 SISTEMA INTEGRADO DE POSTAGEM AUTOMÁTICA")
        print("="*60)
        ...
        # Estatísticas de postagem
        print(f"📊 Total de posts: {self.stats['total_posts']}")
        print(f"✅ Sucessos: {self.stats['successful_posts']}")
        print(f"❌ Falhas: {self.stats['failed_posts']}")
        print(f"💾 Produtos em cache: {len(self.posted_products)}")
        
        if self.stats["last_post_time"]:
            print(f"📅 Último post: {self.stats['last_post_time'].strftime('%d/%m/%Y %H:%M:%S')}")
        
        # Configurações
        print(f"⏰ Intervalo de postagem: {self.posting_interval}")
        print(f"📝 Máximo por ciclo: {self.max_posts_per_cycle}")
        
        print("="*60)

async def main():
    """Função principal"""
    print("🚀 INICIANDO SISTEMA INTEGRADO DE POSTAGEM AUTOMÁTICA")
    print("=" * 60)
