...
class DatabaseManager:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_products_store ON products(store)')
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_products_hash_id ON products(hash_id)')
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_affiliate_clicks_product_id ON affiliate_clicks(product_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_affiliate_clicks_store ON affiliate_clicks(store)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_affiliate_clicks_clicked_at ON affiliate_clicks(clicked_at)')
    ...
    def get_products(self, limit: int = 100, offset: int = 0, 
                     store: Optional[str] = None, category: Optional[str] = None,
    ...
    def get_lowest_price(self, product_id: int, days: int = 90) -> Optional[Dict[str, Any]]:
    ...
    def add_affiliate_click(self, product_id: int, affiliate_url: str, store: str,
    ...
    def get_affiliate_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute('SELECT COUNT(*) as total FROM products WHERE affiliate_url IS NOT NULL AND is_active = 1')
            total_affiliate_links = cursor.fetchone()['total']
            
            # Total de cliques
            cursor.execute('SELECT COUNT(*) as total FROM affiliate_clicks WHERE clicked_at >= ?', (start_date,))
    ...
    def cleanup_old_data(self, days: int = 90):
        ...
        try:
            ...
            cursor.execute('DELETE FROM affiliate_clicks WHERE clicked_at < ?', (cutoff_date,))
            ...
            logger.info(f"Limpeza concluída: {price_history_deleted} preços, {clicks_deleted} cliques, {products_deactivated} produtos desativados")
    ...
    try:
        ...
        for product_data in test_products:
            ...
            if product_id:
                ...
                print(f"   ❌ Falha ao adicionar produto")
