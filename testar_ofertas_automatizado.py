...
class BotSimulado:
    ...
    async def send_photo(self, chat_id, photo, caption, parse_mode, reply_markup):
        ...
        logger.info(f"📸 SIMULAÇÃO: Foto enviada (source={image_source}) para chat {chat_id}")
...
async def testar_estrutura_oferta(oferta: dict, index: int):
    ...
    # Chaves obrigatórias
    chaves_obrigatorias = ["titulo", "preco", "url_produto", "url_afiliado", "loja", "fonte"]
            ...
            if markup:
                ...
                for i, row in enumerate(markup.inline_keyboard):
                    for j, button in enumerate(row):
                        logger.info(f"  Botão {i+1}.{j+1}: {button.text} -> {button.url[:50]}...")
