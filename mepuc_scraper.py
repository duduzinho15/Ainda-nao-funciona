...
class MeuPCScraper:
    ...
    async def buscar_ofertas(
        self, max_paginas: int = 3, max_requests: int = 10
    ) -> List[Dict[str, Any]]:
        """Busca ofertas do MeuPC.net"""
        logger.info(
            f"🚀 Iniciando busca de ofertas no MeuPC.net (max_paginas={max_paginas})"
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Busca concluída. Total de ofertas encontradas: {len(all_ofertas)}"
            )
            return all_ofertas
        ...
        try:
            ...
            async with self.session.get(meupuc_link, allow_redirects=False) as response:
                if response.status == 302:  # Redirecionamento
                    ...
                    if real_url:
                        logger.debug(
                            f"✅ URL real extraída: {meupuc_link} -> {real_url}"
                        )
                        return real_url
                ...
                if meta_refresh:
                    ...
                    if "url=" in content:
                        ...
                        logger.debug(
                            f"✅ URL real extraída via meta refresh: {real_url}"
                        )
                        return real_url
                ...
                if store_link:
                    ...
                    if real_url.startswith("http"):
                        logger.debug(
                            f"✅ URL real extraída via link direto: {real_url}"
                        )
                        return real_url
            ...
            for card in offer_cards:
                try:
                    ...
                    if oferta:
                        ...
                        logger.debug(
                            f"Oferta extraída: {oferta['titulo'][:50]}... - {oferta['preco']}"
                        )
                except Exception as e:
        ...
        try:
            ...
            if not titulo:
                ...
                for text in text_elements:
                    ...
                    if (
                        len(text) > 15
                        and not re.search(r"R\$\s*\d", text)
                        and not re.search(r"\d{1,3}(?:\.\d{3})*,\d{2}", text)
                    ):
                        titulo = text
                        break
            ...
            if not preco:
                price_match = re.search(
                    r"R\$\s*\d{1,3}(?:\.\d{3})*,\d{2}", card.get_text()
                )
                if price_match:
...
async def buscar_ofertas_mepuc(
