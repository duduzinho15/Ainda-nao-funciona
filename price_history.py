...
class PriceHistoryManager:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_points_product ON price_points(product_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_points_store ON price_points(store)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_points_timestamp ON price_points(timestamp)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_trends_product ON price_trends(product_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_alerts_product ON price_alerts(product_id)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_price_alerts_processed ON price_alerts(processed)")
    ...
    def add_price_point(self, product_id: str, store: str, price: float, 
                       original_price: Optional[float] = None, discount_percent: Optional[float] = None,
                       currency: str = "BRL", source: str = "scraper") -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (product_id, store, price, original_price, discount_percent, currency, now.isoformat(), source))
                ...
                conn.commit()
                logger.info(f"Ponto de preço adicionado: {product_id} em {store} - R$ {price}")
                return True
    ...
    def get_price_history(self, product_id: str, store: str, days: int = 30) -> List[PricePoint]:
    ...
    def mark_alert_processed(self, alert_id: int) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("UPDATE price_alerts SET processed = 1 WHERE id = ?", (alert_id,))
                conn.commit()
                return True
        except Exception as e:
    ...
    def _update_price_stats(self, conn: sqlite3.Connection, product_id: str, store: str):
        ...
        try:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                product_id, store, total_points, first_price, last_price, min_price, max_price, avg_price, price_variance, datetime.now().isoformat()
            ))
            
        except Exception as e:
            logger.error(f"Erro ao atualizar estatísticas de preço: {e}")
    
    def _analyze_price_trend(self, conn: sqlite3.Connection, product_id: str, store: str):
        ...
        try:
            ...
            price_data = [(row[0], datetime.fromisoformat(row[1])) for row in cursor.fetchall()]
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                product_id, store, trend_type.value, current_price, price_change, price_change_percent,
                avg_price_30d, min_price_30d, max_price_30d, volatility, datetime.now().isoformat(), confidence
            ))
            
        except Exception as e:
            logger.error(f"Erro ao analisar tendência de preço: {e}")
    
    def _check_price_alerts(self, conn: sqlite3.Connection, product_id: str, store: str, new_price: float):
        ...
        try:
            ...
            if change_percent <= -self.alert_thresholds['significant_drop']:
                alerts_to_create.append(('significant_drop', change_percent, self.alert_thresholds['significant_drop']))
            
            # Aumento significativo
            if change_percent >= self.alert_thresholds['significant_increase']:
                alerts_to_create.append(('significant_increase', change_percent, self.alert_thresholds['significant_increase']))
            
            # Novo mínimo histórico
            cursor = conn.execute("SELECT min_price FROM price_stats WHERE product_id = ? AND store = ?", (product_id, store))
            ...
            if min_price_row and new_price < min_price_row[0]:
                ...
                if min_change <= -self.alert_thresholds['new_low']:
                    alerts_to_create.append(('new_low', min_change, self.alert_thresholds['new_low']))
            
            # Novo máximo histórico
            cursor = conn.execute("SELECT max_price FROM price_stats WHERE product_id = ? AND store = ?", (product_id, store))
            ...
            if max_price_row and new_price > max_price_row[0]:
                ...
                if max_change >= self.alert_thresholds['new_high']:
                    alerts_to_create.append(('new_high', max_change, self.alert_thresholds['new_high']))
            ...
            for alert_type, change, threshold in alerts_to_create:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)
                """, (product_id, store, alert_type, old_price, new_price, change, threshold, datetime.now().isoformat()))
            
        except Exception as e:
            logger.error(f"Erro ao verificar alertas de preço: {e}")
    
    def get_price_analysis(self, product_id: str, store: str, days: int = 30) -> Dict[str, Any]:
    ...
    def _calculate_volatility_analysis(self, history: List[PricePoint]) -> Dict[str, Any]:
    ...
    def _detect_seasonal_patterns(self, history: List[PricePoint]) -> Dict[str, Any]:
        ...
        if len(weekday_avgs) > 1:
            ...
            weekday_variance = statistics.variance(weekday_avgs.values()) if len(weekday_avgs) > 1 else 0
            weekday_volatility = (weekday_variance ** 0.5) / avg_price if avg_price > 0 else 0
    ...
    def get_products_with_significant_changes(self, days: int = 7, min_change_percent: float = 10.0) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                significant_changes = []
                for row in cursor.fetchall():
                    change_percent = ((row[2] - row[4]) / row[4]) * 100 if row[4] > 0 else 0
...
if __name__ == "__main__":
    ...
    if trend:
        print(f"Tendência: {trend.trend_type.value}, Mudança: {trend.price_change_percent:.1f}%")
