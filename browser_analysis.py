...
async def analyze_page(page, url):
    ...
    if network_requests:
        ...
        api_requests = [
            ...
            if "api." in req["name"].lower()
            or "ofertas" in req["name"].lower()
            or "products" in req["name"].lower()
            or "deals" in req["name"].lower()
        ]

        if api_requests:
            page_info["api_requests"] = api_requests
            logger.info(
                f"Encontradas {len(api_requests)} requisições de API relevantes"
            )
...
async def main():
    ...
    try:
        ...
        if requests:
            ...
            logger.info(
                f"Salvas {len(requests)} requisições de rede em network_requests.json"
            )
