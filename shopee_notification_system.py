...
class ShopeeNotificationSystem:
    ...
    async def send_notification(self, title: str, message: str, offers: List = None):
        ...
        try:
            ...
            if offers:
                ...
                for i, offer in enumerate(offers[:self.max_offers_per_notification], 1):
    ...
    async def check_best_offers(self):
        ...
        try:
            ...
            for offer in offers:
                ...
                if (offer.commission_rate >= self.min_commission_rate and 
                    offer.commission_rate > 8.0):  # Comissão alta
    ...
    async def check_daily_deals(self):
        ...
        try:
            ...
            new_offers = []
            for offer in offers:
                offer_id = f"daily_{offer.price}_{offer.commission_rate}_{offer.shop_name}"
    ...
    async def check_price_drops(self):
        ...
        try:
            ...
            for min_price, max_price, title in price_ranges:
                offers = self.shopee_api.get_offers_by_price_range(min_price, max_price, limit=5)
                ...
                if discounted_offers:
                    message = f"Ofertas com desconto entre R$ {min_price} e R$ {max_price}:"
    ...
    async def send_daily_summary(self):
        ...
        try:
            ...
            avg_commission = sum(offer.commission_rate for offer in offers) / total_offers
            best_commission = max(offer.commission_rate for offer in offers)
            
            # Top 3 ofertas do dia
            top_offers = sorted(offers, key=lambda x: x.commission_rate, reverse=True)[:3]
    ...
    async def send_admin_status(self):
        ...
        try:
            ...
            uptime = current_time - self.last_check_time if self.last_check_time else timedelta(0)
    ...
    def schedule_jobs(self):
        ...
        schedule.every(2).hours.do(
            lambda: asyncio.run(self.check_daily_deals())
        )
        
        # Verifica quedas de preço a cada hora
        schedule.every().hour.do(
            lambda: asyncio.run(self.check_price_drops())
        )
        ...
        schedule.every(6).hours.do(
            lambda: asyncio.run(self.send_admin_status())
        )
