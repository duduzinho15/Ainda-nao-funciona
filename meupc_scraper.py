...
async def buscar_ofertas_meupc(
    ...
    try:
        ...
        async with session.get(url_busca, headers=headers) as response:
            ...
            produtos = soup.find_all(['div', 'article'], class_=re.compile(r'product|item|card|oferta|produto'))
            
            if not produtos:
                # Fallback: busca por elementos com links de produto
                produtos = soup.find_all('a', href=re.compile(r'/ofertas/|/produto/|/p/'))
...
def extrair_oferta_produto(produto_element, categoria: str = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        titulo_elem = produto_element.find(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']) or \
                     produto_element.find('span', class_=re.compile(r'title|name|titulo|nome')) or \
                     produto_element.find('a', class_=re.compile(r'title|name|titulo|nome'))
        ...
        # Preço
        preco_elem = produto_element.find(['span', 'div'], class_=re.compile(r'price|preco|valor|custo')) or \
        ...
        # Avaliação
        avaliacao_elem = produto_element.find(['span', 'div'], class_=re.compile(r'rating|avaliacao|score|nota')) or \
        ...
        # Loja
        loja_elem = produto_element.find(['span', 'div'], class_=re.compile(r'store|loja|vendor|vendedor')) or \
                   produto_element.find('img', alt=re.compile(r'logo|store|loja'))
...
async def buscar_ofertas_por_palavra_chave(
    ...
    try:
        ...
        async with session.get(url_busca, headers=headers) as response:
            ...
            produtos = soup.find_all(['div', 'article'], class_=re.compile(r'product|item|card|result|oferta'))
...
async def buscar_ofertas_gaming(
    ...
    try:
        ...
        categorias_gaming = ['placas-de-video', 'processadores', 'memorias', 'ssds', 'monitores', 'perifericos']
        
        todas_ofertas = []
        
        for categoria in categorias_gaming:
            logger.info(f"🔍 Buscando ofertas gaming na categoria: {categoria}")
            ofertas_categoria = await buscar_ofertas_meupc(session, categoria, max_paginas=1)
            
            # Filtra produtos gaming
            ofertas_gaming = []
            for oferta in ofertas_categoria:
                titulo_lower = oferta['titulo'].lower()
                if any(palavra in titulo_lower for palavra in ['gamer', 'gaming', 'game', 'rgb', 'led', 'overclock']):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        ofertas_categoria = await buscar_ofertas_meupc(session, 'processadores', max_paginas=1)
        ...
        ofertas_palavra = await buscar_ofertas_por_palavra_chave(session, 'gamer', max_resultados=5)
