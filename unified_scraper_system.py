...
class UnifiedScraperSystem:
    ...
    def consolidate_results(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"🎯 Total de ofertas únicas consolidadas: {len(unique_ofertas)}")
    ...
    def save_consolidated_results(self, ofertas: List[Dict], filename: str = None) -> str:
    ...
    def generate_status_report(self) -> str:
        ...
        try:
            ...
            for store_name, config in self.stores_config.items():
                ...
                if config["enabled"]:
                    report.append(f"✅ {store_name.upper()}: {ofertas_count} ofertas - Status: {status}")
                else:
                    report.append(f"❌ {store_name.upper()}: DESABILITADA - Status: {status}")
