...
class ConfigService:
    ...
    def _load_config_safe(self, data: Dict[str, Any]) -> SystemConfig:
        ...
        try:
            ...
            if 'search_interval' in filtered_data and isinstance(filtered_data['search_interval'], str):
                filtered_data['search_interval'] = int(filtered_data['search_interval'])
            if 'discount_threshold' in filtered_data and isinstance(filtered_data['discount_threshold'], str):
                filtered_data['discount_threshold'] = float(filtered_data['discount_threshold'])
            if 'commission_threshold' in filtered_data and isinstance(filtered_data['commission_threshold'], str):
                filtered_data['commission_threshold'] = float(filtered_data['commission_threshold'])
            if 'max_products' in filtered_data and isinstance(filtered_data['max_products'], str):
                filtered_data['max_products'] = int(filtered_data['max_products'])
            if 'alert_cooldown' in filtered_data and isinstance(filtered_data['alert_cooldown'], str):
                filtered_data['alert_cooldown'] = int(filtered_data['alert_cooldown'])
            if 'recent_days' in filtered_data and isinstance(filtered_data['recent_days'], str):
                filtered_data['recent_days'] = int(filtered_data['recent_days'])
            if 'max_log_size_mb' in filtered_data and isinstance(filtered_data['max_log_size_mb'], str):
