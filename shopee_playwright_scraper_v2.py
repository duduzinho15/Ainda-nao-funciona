        ...
        try:
            ...
            for i, selector in enumerate(self.product_selectors):
                try:
                    ...
                    if elements:
                        logger.info(
                            f"✅ Selector {i + 1} ({selector}): {len(elements)} elementos encontrados"
                        )
                        ...
                        if len(elements) > 0:
                            ...
                            try:
                                ...
                                if title_text:
                                    logger.info(
                                        f"   📝 Texto do elemento: {title_text[:100]}..."
                                    )
                                ...
                                if inner_html:
                                    logger.info(
                                        f"   🏗️ HTML interno: {inner_html[:200]}..."
                                    )
                ...
                if images:
                    for i, img in enumerate(images[:3]):  # Analisa as primeiras 3
                        try:
                            ...
                            logger.info(
                                f"   Imagem {i + 1}: src={src[:50]}... | alt={alt[:30]}..."
                            )
                        except Exception as e:
