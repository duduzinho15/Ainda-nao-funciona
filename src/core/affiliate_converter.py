...
class AffiliateConverter:
    ...
    def convert_offers_batch(self, offers: list) -> list:
        ...
        self.logger.info(f"Convertidas {len(converted_offers)} ofertas para links de afiliado")
    ...
    def get_affiliate_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "enabled_stores": sum(1 for config in self.affiliate_configs.values() if config["enabled"]),
            "disabled_stores": sum(1 for config in self.affiliate_configs.values() if not config["enabled"]),
            "stores": {}
        }
