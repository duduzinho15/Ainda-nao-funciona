...
try:
    ...
    logger.warning("Módulo 'brotli' não encontrado. O suporte a respostas Brotli estará desabilitado.")
...
def extract_single_oferta(link_element) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        preco_original = ""
        preco_original_elem = link_element.find('span', class_=re.compile(r'line-through'))
...
async def processar_pagina(
    ...
    try:
        ...
        logger.info(f"Página processada: {len(ofertas)} ofertas encontradas, {len(ofertas_filtradas)} após filtros")
    ...
    for oferta in ofertas:
        try:
            ...
            if apenas_frete_gratis:
                ...
                if not any('frete' in tag.lower() and 'grátis' in tag.lower() for tag in tags):
                    continue
            
            # Filtra por lojas oficiais
            if apenas_lojas_oficiais:
                loja = oferta.get('loja', '').lower()
                lojas_oficiais = ['amazon', 'magazine luiza', 'kabum', 'americanas', 'casas bahia']
...
async def buscar_ofertas_promobit(
    ...
    try:
        ...
        logger.info(f"Iniciando busca por ofertas na categoria: {categoria} (URL: {base_url})")
