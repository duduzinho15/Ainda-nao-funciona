        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if response.status_code == 200:
                        ...
                        if 'errors' in result:
                            for error in result['errors']:
                                if 'extensions' in error and 'code' in error['extensions']:
                                    ...
                                    if code == 10020:
                                        logger.error(f"🚨 ERRO 10020: Invalid Authorization Header")
                                        logger.error(f"📋 Mensagem: {error.get('message', 'N/A')}")
                                        logger.error(f"📋 Extensões: {error.get('extensions', {})}")
                                        
                                        # Salva detalhes do erro para análise
                                        with open('erro_10020_detalhes.json', 'w', encoding='utf-8') as f:
                                            json.dump({
                                                'timestamp': time.time(),
                                                'error_details': error,
                                                ...
                                                'app_id': self.app_id,
                                                'app_secret_length': len(self.app_secret)
                                            }, f, indent=2, ensure_ascii=False)
                                        
                                        logger.error("💾 Detalhes do erro 10020 salvos em: erro_10020_detalhes.json")
                                        
                                        # Tenta corrigir o problema de autenticação
                                        if attempt < max_retries:
                                            logger.info("🔄 Tentando corrigir problema de autenticação...")
                                            # Aguarda um pouco antes de tentar novamente
                                            time.sleep(2)
                                            continue
                                        else:
                                            logger.error("❌ Falha após todas as tentativas para erro 10020")
                                            return result
                    ...
                    logger.error(f"❌ Erro 401: Unauthorized - Problema de autenticação")
                    logger.debug(f"📄 Conteúdo da resposta: {response.text[:500]}...")
