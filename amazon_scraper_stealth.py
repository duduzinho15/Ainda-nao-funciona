    ...
    async def simulate_human_behavior(self):
        ...
        try:
            ...
            await self.page.mouse.move(
                x=100 + (time.time() % 100), y=100 + (time.time() % 100)
            )
    ...
    async def scroll_page_stealth(self):
        ...
        try:
            ...
            for i in range(3):
                ...
                await self.page.mouse.move(x=200 + (i * 100), y=300 + (i * 50))
        ...
        try:
                    ...
                    if full_text:
                        ...
                        for line in lines:
                            if len(line) > 10 and not any(
                                word in line.lower()
                                for word in ["r$", "reais", "preço", "price"]
                            ):
                                titulo = line
                                break
                except Exception:
                    ...
                    if preco_elem:
                        ...
                        if preco_text:
                            ...
                            preco_match = re.search(
                                r"R?\$?\s*([\d.,]+)", preco_text, re.IGNORECASE
                            )
                            if preco_match:
            ...
            if not preco:
                ...
                try:
                    ...
                    if full_text:
                        preco_match = re.search(
                            r"R?\$?\s*([\d.,]+)", full_text, re.IGNORECASE
                        )
                        if preco_match:
                            preco = preco_match.group(1)
                except Exception:
            ...
            desconto = None
            try:
                desconto_elem = await product_element.query_selector(
                    '[class*="discount"], [class*="off"], .a-badge-text'
                )
                if desconto_elem:
                ...
                try:
                    ...
                    if full_text:
                        desconto_match = re.search(
                            r"(\d+)%?\s*off|\-(\d+)%?|(\d+)%?\s*desconto",
                            full_text,
                            re.IGNORECASE,
                        )
                        if desconto_match:
                            desconto = int(
                                desconto_match.group(1)
                                or desconto_match.group(2)
                                or desconto_match.group(3)
                            )
                except Exception:
                    pass
    ...
    async def buscar_ofertas(self, max_paginas: int = 3) -> List[Dict]:
        ...
        try:
            ...
            if not produtos_encontrados:
                ...
                try:
                    links_produto = await self.page.query_selector_all(
                        'a[href*="/dp/"]'
                    )
                    if links_produto:
                        ...
                        for link in links_produto:
                            try:
                                produto_pai = await link.query_selector(
                                    'xpath=./ancestor::*[contains(@class, "card") or contains(@class, "item") or contains(@class, "product") or contains(@class, "deal")]'
                                )
                                if (
                                    produto_pai
