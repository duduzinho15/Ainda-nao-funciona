#!/usr/bin/env python3
"""
Script de Instalação para Produção - Sistema de Recomendações
Instala e configura automaticamente o ambiente de produção
"""
import os
import sys
import subprocess
...
class ProductionInstaller:
    """Instalador automatizado para ambiente de produção"""
    ...
    def run_installation(self) -> bool:
        ...
        print("🚀 Instalador de Produção - Sistema de Recomendações")
        print("=" * 60)
    ...
    def _check_prerequisites(self) -> bool:
        ...
        if self.python_version < self.install_config['python_min_version']:
            print(f"❌ Python {self.install_config['python_min_version'][0]}.{self.install_config['python_min_version'][1]}+ requerido")
        ...
        try:
            ...
            free_gb = free // (1024**3)
            if free_gb < 2:
                print(f"❌ Espaço em disco insuficiente. Livre: {free_gb}GB")
                return False
    ...
    def _create_virtual_environment(self) -> bool:
        ...
        try:
            ...
            subprocess.run([sys.executable, '-m', 'venv', str(venv_path)], 
                         check=True, capture_output=True)
            print(f"✅ Ambiente virtual criado: {venv_path}")
            ...
            if not python_path.exists():
                print("❌ Ambiente virtual criado incorretamente")
                return False
    ...
    def _install_dependencies(self) -> bool:
        ...
        try:
            ...
            if self.system == 'windows':
                python_path = self.current_dir / self.install_config['venv_name'] / 'Scripts' / 'python.exe'
                pip_path = self.current_dir / self.install_config['venv_name'] / 'Scripts' / 'pip.exe'
            else:
                python_path = self.current_dir / self.install_config['venv_name'] / 'bin' / 'python'
                pip_path = self.current_dir / self.install_config['venv_name'] / 'bin' / 'pip'
            ...
            requirements_file = self.current_dir / self.install_config['requirements_file']
            if not requirements_file.exists():
                # Usa requirements padrão se não existir
                backup_file = self.current_dir / self.install_config['backup_requirements']
                if backup_file.exists():
                    print(f"   Usando {self.install_config['backup_requirements']}")
                    requirements_file = backup_file
                else:
                    print("❌ Arquivo de requirements não encontrado")
                    return False
            ...
            if result.returncode != 0:
                print(f"❌ Erro na instalação de dependências:")
                print(result.stderr)
                return False
    ...
    def _create_directory_structure(self) -> bool:
        ...
        try:
            ...
            # Cria arquivos vazios necessários
            empty_files = [
                'logs/.gitkeep',
                'backups/.gitkeep',
    ...
    def _generate_configurations(self) -> bool:
        ...
        try:
            ...
            # Cria arquivo de configuração do dashboard
            ...
            print(f"❌ Erro ao gerar configurações: {e}")
            return False
    
    def _generate_production_config(self) -> str:
        """Gera conteúdo do arquivo de configuração de produção"""
        return """# Configuração de Produção - Sistema de Recomendações
# Este arquivo contém configurações sensíveis - NÃO COMMITAR NO GIT
...
"""
    ...
    def _generate_env_example(self) -> str:
        ...
        return """# Arquivo de exemplo para variáveis de ambiente
...
"""
    ...
    def _generate_dashboard_config(self) -> str:
        ...
        return """# Configuração do Dashboard de Produção
# Configurações específicas para o dashboard web
...
"""
    ...
    def _finalize_installation(self):
        ...
        print("- Configure backups automáticos")
        
        print(f"\n🚀 Sistema pronto para produção!")
        print(f"📁 Diretório: {self.current_dir}")
        print(f"🐍 Ambiente: {self.install_config['venv_name']}")
