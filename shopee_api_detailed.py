...
class ShopeeDetailedAPI:
    ...
    def get_product_details(self, product_url: str) -> Optional[Dict[str, Any]]:
        """Extrai detalhes completos do produto usando múltiplas APIs"""
        try:
            logger.info(f"🔍 Extraindo detalhes da Shopee: {product_url}")
            ...
            if item_details:
                logger.info(f"✅ Dados extraídos via API de detalhes: {item_details.get('name', 'N/A')}")
                return item_details
            
            # 2. TENTATIVA: API de busca por ID
            search_results = self._search_by_id(shop_id, item_id)
            if search_results:
                logger.info(f"✅ Dados extraídos via API de busca: {search_results.get('name', 'N/A')}")
                return search_results
            
            # 3. TENTATIVA: API de informações da loja + item
            shop_item_info = self._get_shop_item_info(shop_id, item_id)
            if shop_item_info:
                logger.info(f"✅ Dados extraídos via API da loja: {shop_item_info.get('name', 'N/A')}")
                return shop_item_info
            
            logger.warning("⚠️ Todas as APIs falharam")
            return None
            ...
            for api_url in api_urls:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"   📊 Resposta da API: {api_data}")
    ...
    def _search_by_id(self, shop_id: str, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"   🔍 Buscando por ID: {api_url}")
                ...
                if 'items' in search_data and search_data['items']:
                    ...
                    for item in search_data['items']:
                        if (str(item.get('shopid')) == shop_id and 
                            ...
                            if data.get('title') and data.get('image_url'):
                                logger.info(f"   ✅ Busca por ID funcionou: {data['title']}")
                                return data
    ...
    def _get_shop_item_info(self, shop_id: str, item_id: str) -> Optional[Dict[str, Any]]:
        """API de informações da loja + item"""
        try:
            self._add_delay()
            
            # Tenta obter informações da loja primeiro
            shop_api_url = f"{self.api_base}/shop/get?shopid={shop_id}"
            
            logger.info(f"   🔍 Informações da loja: {shop_api_url}")
            ...
            if response.status_code == 200:
                ...
                # Agora tenta obter informações do item específico
                item_api_url = f"{self.api_base}/item/get?itemid={item_id}&shopid={shop_id}"
                ...
                if item_response.status_code == 200:
                    ...
                    if 'data' in item_data and item_data['data']:
                        ...
                        if data.get('title') and data.get('image_url'):
                            logger.info(f"   ✅ Loja + Item funcionou: {data['title']}")
                            return data
...
def main():
    ...
    print("🧪 TESTANDO API DETALHADA DA SHOPEE")
    print("=" * 60)
