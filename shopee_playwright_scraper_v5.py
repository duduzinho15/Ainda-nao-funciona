#!/usr/bin/env python3
"""
Scraper da Shopee usando Playwright - Versão 5.0 com Análise Dinâmica da Estrutura
Analisa a estrutura real da página e adapta os seletores dinamicamente
"""
...
class ShopeePlaywrightScraperV5(BasePlaywrightScraper):
    """Scraper da Shopee usando Playwright - Versão 5.0 com Análise Dinâmica"""
    ...
    async def analyze_page_structure(self, page_content: str):
        """Analisa a estrutura da página para encontrar padrões de produtos"""
        try:
            logger.info("🔍 Analisando estrutura da página...")
            
            # Procura por padrões comuns de produtos
            patterns = {
                ...
                'discount_patterns': r'(\d+)%\s*(?:off|desconto|redução)'
            }
            ...
            for pattern_name, pattern in patterns.items():
                ...
                if matches:
                    logger.info(f"✅ {pattern_name}: {len(matches)} encontrados")
                    if pattern_name == 'product_links':
                        logger.info(f"   Exemplos: {matches[:3]}")
                else:
                    logger.warning(f"⚠️ {pattern_name}: 0 encontrados")
    ...
    async def find_product_containers(self):
        """Encontra containers de produtos usando múltiplas estratégias"""
        try:
            containers = []
            
            # Estratégia 1: Procura por links de produto
            product_links = await self.page.query_selector_all('a[href*="/product/"], a[href*="/item/"]')
            if product_links:
                logger.info(f"✅ Encontrados {len(product_links)} links de produto")
                containers.extend(product_links)
            
            # Estratégia 2: Procura por elementos com classes suspeitas
            class_selectors = [
                '[class*="product"]',
                '[class*="item"]',
                '[class*="card"]',
            ...
            for selector in class_selectors:
                try:
                    ...
                    if elements:
                        ...
                        # Filtra elementos que não são links de produto
            ...
            # Estratégia 3: Procura por elementos que contenham texto de produto
            try:
                # Procura por elementos que contenham padrões de preço
                all_elements = await self.page.query_selector_all('*')
                for elem in all_elements[:100]:  # Limita a 100 para performance
                    try:
                        text = await elem.text_content()
                        if text and ('R$' in text or 'smartphone' in text.lower() or 'iphone' in text.lower()):
                            if elem not in containers:
                                containers.append(elem)
                    except:
                        continue
            except Exception as e:
                logger.debug(f"⚠️ Erro na busca por texto: {e}")
            
            logger.info(f"🎯 Total de containers encontrados: {len(containers)}")
            return containers
            
        except Exception as e:
            logger.error(f"❌ Erro ao encontrar containers: {e}")
            return []
    
    async def extract_product_from_container(self, container, categoria: str):
        """Extrai informações de produto de um container"""
        try:
            product_info = {
                'titulo': None,
                'preco': None,
            ...
            # Procura por elementos de título dentro do container
            title_selectors = [
                '[class*="title"]',
                '[class*="name"]',
                'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
            ...
            # Se não encontrou título específico, tenta do próprio container
            ...
            # Seleciona o melhor título
            if title_candidates:
                # Filtra títulos muito longos ou muito curtos
            ...
            try:
                ...
                if text and 'R$' in text:
                    ...
                    if price_match:
                        ...
                        product_info['preco'] = "Preço não disponível"
                else:
                    product_info['preco'] = "Preço não disponível"
            except:
                product_info['preco'] = "Preço não disponível"
            ...
            try:
                text = await container.text_content()
                if text:
                    discount_match = re.search(r'(\d+)%\s*(?:off|desconto|redução)', text, re.IGNORECASE)
                    if discount_match:
                        product_info['desconto'] = discount_match.group(1)
            except:
                pass
            
            # Valida se o produto tem informações mínimas
    ...
    async def extract_products_dynamic(self, categoria: str):
        """Extrai produtos usando análise dinâmica da página"""
        try:
            logger.info(f"🔍 Extraindo produtos de {categoria} usando análise dinâmica...")
            ...
            if not containers:
                logger.warning("⚠️ Nenhum container de produto encontrado")
                return []
            ...
            for i, container in enumerate(containers[:20]):  # Limita a 20 produtos
                try:
                    product = await self.extract_product_from_container(container, categoria)
                    if product:
                        products.append(product)
                        logger.debug(f"✅ Produto {i+1} extraído: {product['titulo'][:50]}...")
                except Exception as e:
                    logger.debug(f"⚠️ Erro ao extrair produto {i+1}: {e}")
                    continue
    ...
    async def try_category_access_with_analysis(self, categoria: str):
        ...
        try:
            logger.info(f"🔍 Tentando acesso à categoria: {categoria}")
            ...
            # Aplica user agent móvel (que funcionou nas versões anteriores)
            ...
            if "Entre" in page_content:
                logger.warning("⚠️ Página ainda requer login")
                return []
            
            if len(page_content) < 10000:
                logger.warning("⚠️ Página muito pequena, possível erro")
                return []
            
            logger.info("✅ Categoria acessada com sucesso")
            
            # Faz scroll para carregar conteúdo dinâmico
            await self.scroll_page_smart(2.0)
            await asyncio.sleep(3)
            
            # Extrai produtos usando análise dinâmica
            products = await self.extract_products_dynamic(categoria)
            
            return products
            
        except Exception as e:
            logger.error(f"❌ Erro no acesso à categoria: {e}")
            return []
    
    async def buscar_ofertas_gerais(self):
        """Busca ofertas gerais usando análise dinâmica"""
        todas_ofertas = []
        ...
        try:
            logger.info("🚀 INICIANDO BUSCA DE OFERTAS NA SHOPEE COM PLAYWRIGHT V5.0")
            logger.info("=" * 60)
            ...
            for categoria in self.categorias:
                try:
                    ...
                    # Tenta acesso direto à categoria com análise
                    ofertas_categoria = await self.try_category_access_with_analysis(categoria)
...
async def main():
    ...
    print("🚀 TESTANDO SHOPEE SCRAPER COM PLAYWRIGHT V5.0 (ANÁLISE DINÂMICA)")
    print("=" * 60)
    ...
    for i, oferta in enumerate(ofertas[:10], 1):
        ...
        if oferta.get('imagem'):
            print(f"   🖼️ Imagem: {oferta['imagem'][:50]}...")
