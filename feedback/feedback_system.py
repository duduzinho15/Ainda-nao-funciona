...
class FeedbackSystem:
    ...
    def _init_database(self):
        ...
        try:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_votes_feedback ON votes (feedback_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_comments_feedback ON comments (feedback_id)')
    ...
    def submit_feedback(self, user_id: str, username: str, category: str, title: str, 
                       description: str, priority: str = 'medium', tags: List[str] = None,
        ...
        try:
            ...
            if len(title.strip()) < 5:
                return {'success': False, 'error': 'Título muito curto (mínimo 5 caracteres)'}
            
            if len(description.strip()) < 20:
                return {'success': False, 'error': 'Descrição muito curta (mínimo 20 caracteres)'}
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                feedback_data['id'], feedback_data['user_id'], feedback_data['username'],
                feedback_data['category'], feedback_data['title'], feedback_data['description'],
                feedback_data['priority'], feedback_data['status'], feedback_data['votes'],
                feedback_data['tags'], feedback_data['attachments'], feedback_data['created_at'],
    ...
    def vote_feedback(self, feedback_id: str, user_id: str, vote_type: str = 'upvote') -> Dict[str, Any]:
        """Vota em um feedback"""
        try:
            ...
            if existing_vote:
                ...
                if existing_vote[1] == vote_type:
                    ...
                    cursor.execute('DELETE FROM votes WHERE id = ?', (existing_vote[0],))
            ...
            if self.config['auto_approval_threshold'] > 0:
                cursor.execute('SELECT votes, status FROM feedback WHERE id = ?', (feedback_id,))
                votes, status = cursor.fetchone()
                
                if votes >= self.config['auto_approval_threshold'] and status == 'pending':
                    self._change_feedback_status(feedback_id, 'approved', user_id, 
    ...
    def add_comment(self, feedback_id: str, user_id: str, username: str, content: str) -> Dict[str, Any]:
        """Adiciona comentário a um feedback"""
        try:
            if len(content.strip()) < 5:
                return {'success': False, 'error': 'Comentário muito curto (mínimo 5 caracteres)'}
    ...
    def change_feedback_status(self, feedback_id: str, new_status: str, 
                              changed_by: str, reason: str = None) -> Dict[str, Any]:
        """Muda status de um feedback"""
        try:
            if new_status not in self.feedback_statuses:
                return {'success': False, 'error': 'Status inválido'}
            
            return self._change_feedback_status(feedback_id, new_status, changed_by, reason)
            
        except Exception as e:
            logger.error(f"Falha ao mudar status: {e}")
            return {'success': False, 'error': str(e)}
    
    def _change_feedback_status(self, feedback_id: str, new_status: str, 
                               changed_by: str, reason: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (history_id, feedback_id, old_status, new_status, changed_by, reason or ''))
    ...
    def get_feedback(self, feedback_id: str = None, category: str = None, 
                    status: str = None, priority: str = None, 
                    user_id: str = None, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
    ...
    def get_feedback_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ''')
            top_feedback = [{'title': row[0], 'votes': row[1], 'category': row[2]} 
    ...
    def search_feedback(self, query: str, category: str = None, 
                       status: str = None, limit: int = 20) -> Dict[str, Any]:
        """Pesquisa feedback por texto"""
        try:
            if len(query.strip()) < 3:
                return {'success': False, 'error': 'Query muito curta (mínimo 3 caracteres)'}
    ...
    def cleanup_old_feedback(self) -> Dict[str, Any]:
        ...
        try:
            cutoff_date = datetime.now() - timedelta(days=self.config['feedback_expiry_days'])
...
def main():
    ...
    try:
        while True:
            print(f"\n📋 Menu de Feedback:")
            print(f"   1. 📝 Submeter feedback")
            print(f"   2. 👍 Votar em feedback")
            print(f"   3. 💬 Adicionar comentário")
            print(f"   4. 📊 Ver estatísticas")
            print(f"   5. 🔍 Pesquisar feedback")
            print(f"   6. 🧹 Limpar feedback antigo")
            print(f"   7. ❌ Sair")
            ...
            if choice == '1':
                ...
                category_choice = int(input(f"\n❓ Escolha categoria (1-{len(feedback_system.feedback_categories)}): ")) - 1
                ...
                priority_choice = int(input(f"\n❓ Escolha prioridade (1-{len(feedback_system.priorities)}): ")) - 1
                ...
                print(f"Tipos de voto: upvote, downvote")
                ...
                result = feedback_system.add_comment(feedback_id, user_id, username, content)
