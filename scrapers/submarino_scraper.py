"""
Stub para scraper do Submarino.
Para habilitar, configure as variáveis de ambiente necessárias.

⚠️ AVISO: Scraping do Submarino pode ser bloqueado rapidamente.
...
# Configurações
name = "submarino_scraper"
priority = 85  # Prioridade baixa devido ao risco de bloqueio
...
def enabled() -> bool:
    ...
    if not has_proxy:
        logging.getLogger(f"scraper.{name}").warning(
            "⚠️ Submarino scraper sem proxy - risco alto de bloqueio"
        )
...
async def get_ofertas(periodo: str) -> List[Dict[str, Any]]:
    """
    Stub para coleta de ofertas do Submarino.
    ...
    logger.info("⚠️ Stub do Submarino - implementação não disponível")
...
if __name__ == "__main__":
    async def test():
        print("🧪 Testando stub do Submarino...")
