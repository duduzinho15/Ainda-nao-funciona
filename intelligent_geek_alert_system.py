...
class IntelligentGeekAlertSystem:
            ...
            try:
                ...
                if shopee_products:
                    ...
                    if shopee_products:
                        ...
                        logger.info(f"🔍 Estrutura do primeiro produto Shopee:")
                        logger.info(f"   Atributos: {dir(first_product)}")
                        logger.info(f"   Title: {getattr(first_product, 'title', 'N/A')}")
                        logger.info(f"   Shop name: {getattr(first_product, 'shop_name', 'N/A')}")
                        logger.info(f"   Price: {getattr(first_product, 'price', 'N/A')}")
                        logger.info(f"   Image URL: {getattr(first_product, 'image_url', 'N/A')}")
                        logger.info(f"   Image: {getattr(first_product, 'image', 'N/A')}")
                        logger.info(f"   Product image: {getattr(first_product, 'product_image', 'N/A')}")
        ...
        for i, alert in enumerate(alerts, 1):
            try:
                ...
                if image_url and self._test_image_url(image_url):
                    logger.info(f"🔍 Produto {i}/{len(alerts)}: {product_data.get('title', 'N/A')} - image_url={image_url}")
                    ...
                    try:
                        ...
                        logger.info(f"✅ Alerta {i}/{len(alerts)} postado (com imagem): {product_data.get('title', 'N/A')}")
                    except Exception as e:
                        logger.warning(f"⚠️ Falha ao enviar com imagem, tentando sem: {e}")
                        await self.bot.send_message(
                            chat_id=self.chat_id,
                            text=message,
                            parse_mode='HTML'
                        )
                        logger.info(f"✅ Alerta {i}/{len(alerts)} postado (sem imagem): {product_data.get('title', 'N/A')}")
                else:
                    logger.info(f"🔍 Produto {i}/{len(alerts)}: {product_data.get('title', 'N/A')} - image_url={image_url}")
    ...
    async def _get_real_product_data(self, alert: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if product_data:
                logger.info(f"✅ Dados reais obtidos: {product_data.get('title', 'N/A')}")
                return product_data
            else:
    ...
    def _format_alert_message_with_real_data(self, alert: Dict[str, Any], product_data: Optional[Dict[str, Any]]) -> str:
        """Formata mensagem de alerta com dados reais do produto"""
        try:
            # Usa dados reais se disponíveis, senão usa dados do alerta
            title = product_data.get('title') if product_data else alert.get('title', 'Produto')
