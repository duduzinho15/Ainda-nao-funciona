...
def generate_hashes_for_existing_offers(cursor: sqlite3.Cursor):
    ...
    try:
        ...
        logger.info(f"🔄 Gerando hashes para {len(offers_without_hash)} ofertas existentes...")
        ...
        for offer in offers_without_hash:
            try:
                ...
                if hash_value:
                    ...
                    logger.warning(f"⚠️ Não foi possível gerar hash para oferta: {offer[2]}")
...
def validate_migration(cursor: sqlite3.Cursor):
    ...
    try:
        ...
        if 'idx_ofertas_offer_hash' not in index_names:
            logger.error("❌ Validação falhou: índice idx_ofertas_offer_hash não existe")
            return False
        ...
        if offers_without_hash > 0:
            logger.warning(f"⚠️ {offers_without_hash} ofertas ainda não possuem offer_hash")
        else:
...
def main():
    ...
    try:
        ...
        cursor.execute('SELECT COUNT(*) FROM ofertas WHERE offer_hash IS NOT NULL AND offer_hash != ""')
