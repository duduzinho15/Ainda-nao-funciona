                ...
                try:
                    ...
                    async with session.get(url, timeout=30) as response:
                        if response.status != 200:
                            logger.warning(f"⚠️ Erro ao acessar {url}: {response.status}")
                            continue
                        ...
                        if len(ofertas) == 0:
                            with open('pelando_debug_simple.html', 'w', encoding='utf-8') as f:
                                f.write(html)
                            logger.info("✅ HTML salvo em pelando_debug_simple.html para debug")
...
def extrair_oferta_produto(produto_element) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        oferta = {
            'titulo': titulo,
            'url_produto': url_produto or f"https://www.pelando.com.br",
            'url_fonte': f"https://www.pelando.com.br",
            'preco': preco_atual,
