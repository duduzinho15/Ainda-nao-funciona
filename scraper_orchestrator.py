...
class ScraperOrchestrator:
    ...
    def get_scraper_instance(
        self, store_name: str, use_stealth: bool = None, use_advanced: bool = False
    ):
    ...
    async def scrape_store_async(
        ...
        try:
            ...
            if store_name == "Promobit":
                ...
                if AWIN_API_AVAILABLE:
                    ofertas = await buscar_ofertas_awin(
                        max_ofertas=max_ofertas, min_desconto=15
                    )
                else:
                    log_error(
                        f"SCRAPING {store_name}",
                        Exception("API da Awin não disponível"),
                        "Configuração inválida",
                    )
                    logger.error("❌ API da Awin não disponível")
                    return store_name, []
                ...
                if MERCADOLIVRE_AVAILABLE:
                    ...
                    log_error(
                        f"SCRAPING {store_name}",
                        Exception("Scraper do Mercado Livre não disponível"),
                        "Configuração inválida",
                    )
                    logger.error("❌ Scraper do Mercado Livre não disponível")
                    return store_name, []
            else:
                # Outras lojas usam classes
                scraper = self.get_scraper_instance(
                    store_name, use_stealth, use_advanced
                )
                if not scraper:
                    log_error(
                        f"SCRAPING {store_name}",
                        Exception("Scraper não disponível"),
                        "Configuração inválida",
                    )
                ...
                if hasattr(scraper, "buscar_ofertas"):
                    if asyncio.iscoroutinefunction(scraper.buscar_ofertas):
                        ...
                        ofertas = await loop.run_in_executor(
                            self.executor, scraper.buscar_ofertas
                        )
                    ...
                    if asyncio.iscoroutinefunction(scraper.scrape_all_categories):
                        ...
                        loop = asyncio.get_event_loop()
                        ofertas = await loop.run_in_executor(
                            self.executor, scraper.scrape_all_categories
                        )
                else:
                    log_error(
                        f"SCRAPING {store_name}",
                        Exception("Método não encontrado"),
                        "Interface incompatível",
                    )
                    logger.error(
                        f"❌ Método de scraping não encontrado em {store_name}"
                    )
                    return store_name, []
            ...
            logger.info(
                f"✅ {store_name}: {len(ofertas)} ofertas em {elapsed_time:.2f}s"
            )
    ...
    async def scrape_all_stores(
        self, use_stealth: bool = None, use_advanced: bool = False
    ) -> Dict[str, List[Dict]]:
        ...
        logger.info(
            f"🎯 Iniciando scraping de todas as lojas (Stealth: {use_stealth}, Advanced: {use_advanced})"
        )
        ...
        logger.info(
            f"🎯 Scraping concluído: {total_ofertas} ofertas de {len(all_ofertas)} lojas"
        )
        return all_ofertas

    def merge_and_filter_ofertas(
        self,
        all_ofertas: Dict[str, List[Dict]],
        min_desconto: int = 0,
    ...
    async def run_complete_scraping(
        ...
        try:
            log_info(
                "ORQUESTRADOR",
                f"Iniciando scraping completo (Stealth: {use_stealth}, Advanced: {use_advanced})",
            )
            ...
            log_info(
                "ORQUESTRADOR",
                f"Scraping completo finalizado: {len(final_ofertas)} ofertas filtradas",
            )
