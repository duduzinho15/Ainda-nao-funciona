    ...
    try:
        ...
        async with session.get(url_busca, headers=headers) as response:
            ...
            produtos = soup.find_all(
                ["div", "article"], class_=re.compile(r"product|item|card|oferta")
            )
...
def extrair_oferta_produto(
    ...
    try:
        # Título do produto
        titulo_elem = produto_element.find(
            ["h1", "h2", "h3", "h4", "h5", "h6"]
        ) or produto_element.find("span", class_=re.compile(r"title|name|titulo"))
        ...
        avaliacao_elem = produto_element.find(
            ["span", "div"], class_=re.compile(r"rating|avaliacao|score")
...
async def buscar_ofertas_por_palavra_chave(
    ...
    try:
        ...
        async with session.get(url_busca, headers=headers) as response:
            ...
            produtos = soup.find_all(
                ["div", "article"], class_=re.compile(r"product|item|card|result")
            )
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        ofertas_categoria = await buscar_ofertas_buscape(
            session, "celulares", max_paginas=1
        )
        ...
        ofertas_palavra = await buscar_ofertas_por_palavra_chave(
            session, "smartphone", max_resultados=5
        )
